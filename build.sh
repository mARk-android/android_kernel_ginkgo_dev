...
# Line added to changelog    
    ...
    sed  -i '1i\mARkOS kernel custom Q build-'${VERSION} AnyKernel3/changelog
    sed  -i '1i\=======================================' AnyKernel3/changelog
...
#    echo -e ${dim}"Compiling with "${txtbld}${grn}${blink}"$KBUILD_COMPILER_STRING"${txtrst};
...
if [ -f "$out/arch/arm64/boot/Image.gz-dtb" ] && [ -f "$out/arch/arm64/boot/dtbo.img" ]; then
	    ...
	    ZIPNAME="boot•mARkOS•$VERSION•custom•A10•HMNote8•8T-$(date '+%Y%m%d-%H'00).zip"    
